...
def login(username, password):
    ...
    session.post('https://globalchallenge.virginpulse.com/log-in', data={
        'username': username,
        'password': password
    })
    return session
...
def get_other_teams(session):
    res = session.post('https://globalchallenge.virginpulse.com/event/map/organisation-teams')
    return res.json()
...
my_team = {'name': my_team['Name'], 'steps': int(my_team['StepTotalFormatted'].replace('.', ''))}
teams = [my_team]
